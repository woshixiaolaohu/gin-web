...
type SubInitializer interface {
	InitializerName() string // 不一定单独代表一个表 所以改成了更加宽泛的语义
...
type TypedDBInitHandler interface {
	EnsureDB(ctx context.Context, conf *request.InitDB) (context.Context, error) // 建库 失败属于 fatal error 要 panic
...
func (initDBService *InitDBService) InitDB(conf request.InitDB) (err error) {
	...
	if len(initializers) == 0 {
		return errors.New("无可用初始化进程，请检查初始化是否已执行完成")
	}
	/*
