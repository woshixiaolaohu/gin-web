...
func (s *SystemApi) ReloadSystem(c *gin.Context) {
	err := utils.Reload()
	if err != nil {
		global.GVA_LOG.Error("系统重启失败", zap.Error(err))
		response.FailWithMessage("系统重启失败", c)
		return
	}
	response.OkWithMessage("系统重启成功", c)
}
...
func (s *SystemApi) GetServerInfo(c *gin.Context) {
	...
	response.OkWithDetailed(gin.H{"server": server}, "获取服务器信息成功", c)
}
