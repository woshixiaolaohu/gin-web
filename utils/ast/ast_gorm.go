...
func AddRegisterTablesAst(path, funcName, pk, varName, dbName, model string) {
	modelPk := fmt.Sprintf("github.com/woshixiaolaohu/gin-web/server/model/%s", pk)
...
// addAutoMigrate 为db库变量增加 AutoMigrate 方法
func addAutoMigrate(astBody *ast.BlockStmt, dbname string, pk string, model string) {
	...
	ast.Inspect(astBody, func(node ast.Node) bool {
		// 首先判断需要加入的方法调用语句是否存在 不存在直接走到下方逻辑
		switch n := node.(type) {
		case *ast.CallExpr:
		...
		return true
		// 然后判断 pk.model是否存在 如果存在直接跳出 如果不存在 则向已经找到的方法调用语句的node里边push一条
	})
	if flag {
...
// NeedAppendModel 为 automigrate 增加实参
func NeedAppendModel(callMode ast.Node, pk string, model string) bool {
	flag := true
	ast.Inspect(callMode, func(node ast.Node) bool {
		switch n := node.(type) {
